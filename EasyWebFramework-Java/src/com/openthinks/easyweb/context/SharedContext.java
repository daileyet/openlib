...
public abstract class SharedContext implements WebContext {
	...
	public <T, E extends T> T lookup(final Class<T> searchType,
		...
		if (searchType != null) {
			...
			if (object == null) {
				if (instancewrapper == null)
					instancewrapper = (InstanceWrapper<E>) InstanceWrapper
							.build(searchType);
				...
				for (Constructor<E> c : constructors) {
					try {
						...
						WebUtils.checkParamNumbers(paramTypes, args);
						WebUtils.checkParamTypes(paramTypes, args);
	...
	protected static class InstanceWrapper<E> {
		...
		public static <T> InstanceWrapper<T> build(Class<T> instanceType,
				Object... objects) {
			Object owner = null;
