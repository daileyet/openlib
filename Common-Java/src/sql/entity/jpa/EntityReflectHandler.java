...
public class EntityReflectHandler extends JPAReflectHandler implements
		IReflectHandler {
	...
	public <T> boolean handColumnField(T entity, String columnName,
		...
		boolean isSuccess = super.handColumnField(entity, columnName,
				columnValue);
		if (isSuccess) {
			return true;
		}
		if (entity instanceof sql.entity.Entity) {
			try {
				((sql.entity.Entity) entity).set(columnName, columnValue);
				isSuccess = true;
			} catch (Exception e) {
				isSuccess = false;
			}
	...
	public <T> ColumnAttributeMapping parseEntityClass(Class<T> entityClass) {
		ColumnAttributeMapping columnAttributeMapping = super
				.parseEntityClass(entityClass);

		if (columnAttributeMapping.isEmpty()
				&& Entity.class.isAssignableFrom(entityClass)) {
			try {
				Entity entityInstance = (Entity) entityClass.newInstance();
				for (ColumnAttribute columnA : entityInstance
						.getColumnAttributes()) {
